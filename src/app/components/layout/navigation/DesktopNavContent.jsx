...
const DesktopNavContent = ({scrolled, pathname, transitionRouter, pageAnimation}) => {
    ...
    return (
        <div className={`flex-col items-end hidden lg:flex ${scrolled ? 'space-y-1 py-1' : 'space-y-2'}`}>
        <motion.ul
            initial={"hidden"}
            animate="visible"
                    ...
                    <AnimatePresence>
                        {showServicesPanel && (
                            <motion.div
                                ...
                                className="fixed flex justify-center items-center min-h-[80vh] top-0 left-0 w-screen pt-32 pb-8 bg-darkgray/90 backdrop-blur-md z-[-1]"
                                onMouseEnter={() => setShowServicesPanel(true)}
                                onMouseLeave={() => setShowServicesPanel(false)}
                            >
                                <PageContainer>
                                    <div className='grid grid-cols-2'>
                                        ...
                                        <div className="relative w-full aspect-video">
                                            <AnimatePresence mode="wait">
                                                ...
                                                exit={{ opacity: 0, y: 20 }}
                                                transition={{ duration: 0.2, ease: [0.68, 0, 0.32, 1] }}
                                                className="absolute inset-0"
                                                >
                                                <Image alt='Freisinger Leistung' src={activeImage} className="object-cover" fill />
                                                </motion.div>
                                            </AnimatePresence>
                                        </div>
                                    </div>
